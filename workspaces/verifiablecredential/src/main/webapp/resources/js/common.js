...
function setDataStep1(){
	this.tempObject.issuedPlaceCd = "10001";
	document.commonCode.issuedPlaceCd.value = value; 
}

function onStep2(){
	message = "{\\\"address\\\":\\\"1234\\\",\\\"issuedplace\\\":\\\"1234\\\",\\\"kornm\\\":\\\"woori\\\",\\\"residno\\\":\\\"1234\\\",\\\"id\\\":\\\"did:woori:10330\\\",\\\"issueddate\\\":\\\"2019.4.4\\\"}";
	//showStep5Load(message);
	//return;
...
function setDataStep2(){
	var value = $("input:radio[name=certiCd]:checked").val();
...
function onStep3(){
	if($("input:radio[name=certiCd]").is(":checked")){
		setDataStep2();
	}else{
		...
		type:"POST"
			...
			,success:function(data){
				$.each(data, function(idx, item){
					...
					//checkbox
					html+="<td>";
					html+="<input type='radio' name='certiCd' value='"+item.cd+"'/>";
					html+="</td>";
				...
				if("" != tempObject.getCertiCd()){
					$("input:radio[name=certiCd]:radio[value='"+tempObject.getCertiCd()+"']").prop("checked",true);
		    	}
			}
			,error:function(xhr, status, error){
		...
		type:"POST"
			...
			,success:function(data){
				$.each(data, function(idx, item){
					...
					//checkbox
					html+="<td>";
					html+="<input type='checkbox' name='orderNo' value='"+item.fieldNm+"' data-fieldNm='"+item.certiCd+"_"+item.fieldNm+"' onclick='step3checkClick(this);' />";
					html+="</td>";
...
function showStep5Load(jsonStr) {
	$(".nav-tabs").addClass("d-none");
	 
    $(".tab-pane").removeClass("active fadein fade show");
    $("#step5").addClass("tab-pane fade fadein active show");
}

function onStep4Load(jsonStr){
	...
	var qrJsonArray = new Array();
	qrJsonArray.push("qr");
	qrJsonArray.push(jsonStr);
	
	var qrJsonObject = new Object();
	qrJsonObject.args = qrJsonArray
	
	send(JSON.stringify(qrJsonObject));
}

function send(jsonStr) {// JSON 문자열을 서버로 전송한다
...
function step3checkClick(check){
	alert($("input:checkbox[name=orderNo]:checked").length);
	alert($("input:checkbox[name=orderNo]").length);
